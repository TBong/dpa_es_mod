label prolog:
    ...
    elt "Намерен завтра обратится к Федеральному Собранию с предложением совместно уточнить рамочные условия для проведения переговоров."
    ...
    elt "Обязываю всех должностных лиц, ответственных за проведение мероприятий по восстановлению в Чеченской Республике конституционного порядка,{w} не применять насилия против мирного населения,{w} взять его под свою защиту."
    ...
    "Однако корреспондентам, освещающим этот конфликт, было ясно:{w} Ельцинское решение приведет к катастрофе -{w} прежде всего потому, что российские вооруженные силы представляли собой пугающее скопище недисциплинированных людей."
    
    hide gazeta1 with dissolve2
    play music song_ep fadein 2
    $ set_mode_nvl()
    ...
    "Мне – всего 23, я молод и беззаботен, и это мое боевое крещение."
    stop music fadeout 2
    $ set_mode_adv()
    nvl clear
    ...
    "Нас построили, и один из них задвинул речь, из которой мы узнали, что прибыли в Моздок (а мы думали – в Сан-Франциско!)."
    ...
    "Кряхтя и превозмогая немочь, я полез в вещмешок."
