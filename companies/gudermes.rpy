...
label gudermes_prolog:
    "Гудермес."
    $ dpa_new_chapter(0, "Пролог: \"Гудермес\"")
    ...
    $ dpa_new_chapter(0, "Гудермес день 0: \"Сон\"")
...
label gudermes_day1:
    stop music fadeout 2
    $ renpy.pause(3)
    $ dpa_new_chapter(0, "Гудермес день 1: \"Начало\"")
    scene angar with dissolve
    ...
    gg "Невыспался малех."
    gen "Верю, такая же бодяга. Как зовут то хоть?"
    gg "Саша."
    gen "Меня Гена, приятно."
    $ dpa_rename_character_from_list("gen_name",gen_names,1)
    $ gen_fp += 1
    ...
    show rs furious
    rs "КАКОГО ХЕРА?!{w} Вы как сюда попали, блять?!{w} Каки блядским образом?!"
    "Мы опешили, непонимая что делать"
    rs "Ало, вы в уши долбитесь? Ответ где."
    "Гена решил срезать углы."
    gen "Нас с Маздока отправили. Посадили в вагон и мы поехали."
    rs "Ебаные разгельдяи, нихера не могут сделать нормально. В любом случае, за мной дуйте, сейчас с вами разберемся."
    $ dpa_new_chapter(0, "Гудермес день 1: \"Вокзал\"")
    return