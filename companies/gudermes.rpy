label gudermes:
    ...
    $ new_chapter(0, u"Гудермес день 0: \"Воспоминания о былом\"")
    ...
    nvl clear
    hide black
    $ new_chapter(0, u"Гудермес день 1: \"Утро перед отправлением\"")
    ...
    $ new_chapter(0, u"Гудермес день 1: \"Путь\"")
    ...
    # Сюда нужно фото вагона
    ofic "Загружайтесь."
    "Мы с Геной и ещё тремя парнями прыгнули в вагон.{w} Возможно я единственный заметил что с губ офицера сорвался шепот..."
    ofic "{color=#262626}С Богом...{/color}"
    "Состав тронулся ."
    "Напротив меня седел Гена."
    gen "Тоже это увидел?"
    gg "Что \"это\"?"
    gen "Шепот офицера."
    ...
    gg "Приму к сведенью."
    #Тут вид из вагона на природу
    "Я оторвал залипнувший взгляд от пола и глянул на вид открывающийся с окна."
    "Это было прекрастное зрелище, вид заварживал, поле дикого мака жгуче било своей насыщеностью в глаза, синее бескрайнее небо без единого облочка и горы, величественные горы. Вдоль них то и лежал наш путь."
    # Сюда можно пикчу как они собрались играть
    ...
    "Карты раскидали на троих, остальным надаело играть. Ну а как ту не надоест, когда тебя в дураках оставляют."
    ...
    sol "Судьба так сыграла, вырос я в таком месте, где все так общаются, если не все то большенство. Как ты понял, на кичи я не чалился."
    ...
    "Веселое лицо Гены куда то испарилось, вместо него тепел был Генадий, серьёзный от мозга костей, по крайней мере по его лицу можно было только это сказать."
    ...
    "И вдруг огонек опять зажогся, лико превратилось в моьску, Гена \"вернулся обратно\". Почему то я абсолютно уверен, что дело не в проигрыше."
